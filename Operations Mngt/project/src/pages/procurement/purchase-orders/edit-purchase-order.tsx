...
export function EditPurchaseOrder() {
  ...
  const { usePurchaseOrder, useUpdatePurchaseOrder } = usePurchaseOrders();
  const { data: po, isLoading: isLoadingPO } = usePurchaseOrder(id!);
  const { mutate: updatePO, isLoading: isUpdating } = useUpdatePurchaseOrder();
  ...
  const defaultValues = {
    ...
    requiredByDate: new Date(po.requiredByDate),
  ...
  return (
    <div className="space-y-6 p-6">
      <FormContainer
        ...
        cancelText="Cancel"
        onCancel={() => navigate(`/procurement/purchase-orders/${po.id}`)}
        showReset
      >
