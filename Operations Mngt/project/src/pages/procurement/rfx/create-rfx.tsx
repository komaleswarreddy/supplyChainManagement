...
const defaultValues = {
  ...
  settings: {
    ...
    allowSupplierQuestions: true,
    visibleToAllSuppliers: false,
    requireNda: true,
  },
} as const;
...
export function CreateRfx() {
  const navigate = useNavigate();
  const { useCreateRfx } = useRfx();
  const { mutate: createRfx, isLoading } = useCreateRfx();

  const onSubmit = async (data: z.infer<typeof rfxSchema>) => {
    const formData: RfxFormData = {
      ...data,
    };

    createRfx(formData, {
      onSuccess: () => {
        navigate('/procurement/rfx');
      },
    });
  };
  ...
  return (
    <div className="space-y-6 p-6">
      <FormContainer
        title="Create RFx"
        description="Create a new RFx event"
        schema={rfxSchema}
        defaultValues={defaultValues}
        onSubmit={onSubmit}
        submitText={isLoading ? 'Creating...' : 'Create RFx'}
        cancelText="Cancel"
        onCancel={() => navigate('/procurement/rfx')}
        showReset
      >
