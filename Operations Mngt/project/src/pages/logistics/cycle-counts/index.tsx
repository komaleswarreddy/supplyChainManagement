...
import type { CycleCount } from '@/types/logistics';
import { ArrowUpDown, Eye, Plus, ClipboardList } from 'lucide-react';
import DatePicker from 'react-datepicker';
import "react-datepicker/dist/react-datepicker.css";

const columnHelper = createColumnHelper<CycleCount>();
...
export function CycleCountList() {
  ...
  const columns = [
    columnHelper.accessor('countNumber', {
      header: ({ column }) => (
    ...
    columnHelper.accessor('abcClass', {
      ...
      cell: (info) => info.getValue() ? (
        <Badge variant={
          info.getValue() === 'A' ? 'success' : 
          info.getValue() === 'B' ? 'warning' : 
          'default'
        }>
          {info.getValue()}
        </Badge>
    ...
    columnHelper.accessor('id', {
      ...
      cell: (info) => (
        <Button
          ...
          onClick={(e) => {
            e.stopPropagation();
            navigate(`/logistics/cycle-counts/${info.getValue()}`);
          }}
        >
  ...
  const { useCycleCounts } = useLogistics();
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <DataTable
        table={table}
        isLoading={isLoading}
        onRowClick={(row) => navigate(`/logistics/cycle-counts/${row.id}`)}
      />
    </div>
  );
}